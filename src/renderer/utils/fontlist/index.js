...
const RESOURCES_PATH = isDevelopment
  ? path.join(process.resourcesPath, '../../../../extraResources/fontlist/libs')
  : path.join(__dirname, 'extraResources/fontlist/libs');

const getLibsPath = (...paths) => {
...
switch (platform) {
  case 'darwin':
    getFontsFunc = require(getLibsPath('darwin'))
    break
  case 'win32':
    getFontsFunc = require(getLibsPath('win32'))
    break
  case 'linux':
    getFontsFunc = require(getLibsPath('linux'))
    break
  default:
