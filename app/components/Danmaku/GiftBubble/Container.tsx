...
const GiftContainer = (props: GiftContainerProps) => {
  const { lists, giftMap, comboMap } = props;
  const transitions = useTransition(lists, list => list.id, {
    from: { left: '-100%' },
    enter: { left: '0%' },
    leave: { left: '-100%' }
  });
  ...
  return (
    <Wrapper>
      {transitions.map(tprops => {
        const { item, key, props } = tprops;
        const { msg } = item;
        ...
        return (
          <GiftBubbleItem {...item} key={key} id={item.id} style={props}>
                  ...
                  <div className="gift-pic-count p-relative">
                    ...
                    {/* 包含连击效果: count = giftCount * superBatchGiftNum */}
                    {
                      <div className="combo-amount dp-i-block">
                        ...
                        <span className="multiply dp-i-block" />
                        {Multiply(msg.batchComboId, giftCount * superBatchGiftNum)}
                      </div>
                    }
                  </div>
                </div>
